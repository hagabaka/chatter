...
accessor methods (+property+, +property=+) as aliases to (+getProperty+,
+setProperty+). But this is not directly possible with properties
that take arguments, for example, +TableWidget#getCellText(x, y)+,
+TableWidget#setCellText(x, y)+. By using
